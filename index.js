...
const tookAsync = async (id, offset, limit) => {
	...
	try {
		...
		// let url = `http://staging.quran.com:3000/api/v3/chapters/${id}/verses?recitation=1&translations=33&language=id&text_type=words&offset=${offset}&limit=${limit}`;
		let url = ``;
...
const wrote = (id, response) => {
	...
	jsonfile.writeFile(`./qapi/chapters/verses/18_id-${id}.json`, data, {
		spaces: 2
	}, function (err, result) {
		if (err) console.log('error', err);
	});
		...
		if (v.verses_count > 50) {
			...
			for (let i = 1; i <= iteration; i++) {
				...
				if (i == iteration) {
					...
					ink.push(...tooki.body.verses);
					// console.log(...await tookAsync(v.chapter_number, go - 50, go));
				}
				go += 50
				// console.log(things)
